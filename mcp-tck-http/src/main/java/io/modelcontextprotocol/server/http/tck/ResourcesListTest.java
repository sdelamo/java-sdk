...
public class ResourcesListTest {
	...
	@Test
	public void resourcesListTest() throws Exception {
		McpHttpServer server = McpHttpServer.getDefault();
		HttpRequest request = HttpRequestUtils.POST(server, RESOURCES_LIST);
		HttpClient client = HttpClient.newBuilder().build();
		HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
		assertEquals(200, response.statusCode());
		String json = response.body();
		JSONAssert.assertEquals(RESOURCES_LIST_RESULT, json, true);
	}
